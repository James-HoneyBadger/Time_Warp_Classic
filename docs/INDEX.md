# Time Warp Classic - Complete Documentation Suite

Welcome to the Time Warp Classic comprehensive documentation. This is your complete guide to mastering the multi-language retro IDE.

## 📚 Documentation Categories

### Getting Started
- **[QUICK_START.md](QUICK_START.md)** - Launch and run your first program in 5 minutes
- **[../SETUP.md](../SETUP.md)** - Installation, setup, and troubleshooting guide

### User Documentation
- **[LANGUAGE_TUTORIALS.md](user/LANGUAGE_TUTORIALS.md)** - Learn all 9 programming languages

### Developer Documentation
- **[TECHNICAL_REFERENCE.md](dev/TECHNICAL_REFERENCE.md)** - Architecture, API, and implementation details

### Language Guides
- **[BASIC_REFERENCE.md](languages/BASIC_REFERENCE.md)** - Complete BASIC language reference
- **[PYTHON_REFERENCE.md](languages/PYTHON_REFERENCE.md)** - Python language reference
- **[JAVASCRIPT_REFERENCE.md](languages/JAVASCRIPT_REFERENCE.md)** - JavaScript language reference
- **[PASCAL_REFERENCE.md](languages/PASCAL_REFERENCE.md)** - Pascal language reference
- **[PROLOG_REFERENCE.md](languages/PROLOG_REFERENCE.md)** - Prolog language reference
- **[FORTH_REFERENCE.md](languages/FORTH_REFERENCE.md)** - Forth language reference
- **[PERL_REFERENCE.md](languages/PERL_REFERENCE.md)** - Perl language reference
- **[LOGO_REFERENCE.md](languages/LOGO_REFERENCE.md)** - Logo language reference
- **[PILOT_REFERENCE.md](languages/PILOT_REFERENCE.md)** - PILOT language reference

### Project Information
- **[../examples/README.md](../examples/README.md)** - Example programs and code samples
- **[TROUBLESHOOTING.md](TROUBLESHOOTING.md)** - Common issues and solutions
- **[FAQ.md](FAQ.md)** - Frequently asked questions

### Release Publishing (Maintainers)
- **[QUICK_START.md](QUICK_START.md)** - Tag-based release commands for TestPyPI and PyPI
- **[../.github/workflows/publish-testpypi.yml](../.github/workflows/publish-testpypi.yml)** - Prerelease publish workflow
- **[../.github/workflows/publish-pypi.yml](../.github/workflows/publish-pypi.yml)** - Stable publish workflow

## 🚀 Quick Navigation

### I want to...

**Get started immediately**
→ [QUICK_START.md](QUICK_START.md)

**Learn a programming language**
→ [LANGUAGE_TUTORIALS.md](user/LANGUAGE_TUTORIALS.md)

**Find language syntax reference**
→ [languages/](languages/) directory

**Understand the architecture**
→ [TECHNICAL_REFERENCE.md](dev/TECHNICAL_REFERENCE.md)

**Extend or modify the IDE**
→ [TECHNICAL_REFERENCE.md](dev/TECHNICAL_REFERENCE.md)

**Troubleshoot a problem**
→ [TROUBLESHOOTING.md](TROUBLESHOOTING.md)

**Find answers to common questions**
→ [FAQ.md](FAQ.md)

**Run example programs**
→ [../examples/README.md](../examples/README.md)

**Publish a package release**
→ [QUICK_START.md](QUICK_START.md) (Release Publishing section)

## 📖 Reading Recommendations

### For Absolute Beginners
1. Read [QUICK_START.md](QUICK_START.md)
2. Try running an example in [../examples/](../examples/)
3. Follow [LANGUAGE_TUTORIALS.md](user/LANGUAGE_TUTORIALS.md) - BASIC section

### For Experienced Programmers
1. Read [QUICK_START.md](QUICK_START.md) for IDE overview
2. Check language reference for your chosen language in [languages/](languages/)
3. Review [TECHNICAL_REFERENCE.md](dev/TECHNICAL_REFERENCE.md)
4. Explore [examples/](../examples/) for advanced patterns

### For IDE Developers
1. Read [TECHNICAL_REFERENCE.md](dev/TECHNICAL_REFERENCE.md)
2. Check source code in [core/](../core/) directory

## 🎯 Learning Paths

### Path 1: Programming Fundamentals (Beginner)
```
QUICK_START.md
  ↓
BASIC_REFERENCE.md
  ↓
Try examples/basic/comprehensive_demo.bas
  ↓
PYTHON_REFERENCE.md
  ↓
Try examples/python/comprehensive_demo.py
  ↓
USER_GUIDE.md (full)
```

### Path 2: Multi-Language Explorer (Intermediate)
```
QUICK_START.md
  ↓
LANGUAGE_TUTORIALS.md
  ↓
Try demos for languages 1-3
  ↓
Each language reference file
  ↓
Advanced examples in examples/
```

### Path 3: IDE Developer (Advanced)
```
TECHNICAL_REFERENCE.md
  ↓
Review core/interpreter.py
  ↓
Study core/languages/ implementations
  ↓
Study gui/ package
```

## 📋 File Organization

```
docs/
├── INDEX.md (this file)
├── README.md
├── QUICK_START.md
├── TROUBLESHOOTING.md
├── FAQ.md
├── user/
│   └── LANGUAGE_TUTORIALS.md
├── dev/
│   └── TECHNICAL_REFERENCE.md
└── languages/
    ├── BASIC_REFERENCE.md
    ├── PYTHON_REFERENCE.md
    ├── JAVASCRIPT_REFERENCE.md
    ├── PASCAL_REFERENCE.md
    ├── PROLOG_REFERENCE.md
    ├── FORTH_REFERENCE.md
    ├── PERL_REFERENCE.md
    ├── LOGO_REFERENCE.md
    └── PILOT_REFERENCE.md
```

## 🔍 Search Tips

- Use Ctrl+F to search within each document
- Check the Table of Contents at the top of longer documents
- Look for **Bold text** highlighting important concepts
- Follow `code formatting` for syntax examples

## 🆘 Need Help?

1. **Quick question?** → Check [FAQ.md](FAQ.md)
2. **Something not working?** → See [TROUBLESHOOTING.md](TROUBLESHOOTING.md)
3. **Want to learn a language?** → Go to [languages/](languages/) directory
4. **Need IDE features?** → Read [QUICK_START.md](QUICK_START.md)
5. **Want to extend?** → Study [dev/TECHNICAL_REFERENCE.md](dev/TECHNICAL_REFERENCE.md)

## 📞 Support Resources

- **Official Repository:** https://github.com/James-HoneyBadger/Time_Warp_Classic
- **Issue Tracker:** Report bugs and feature requests
- **Discussions:** Community Q&A and knowledge sharing
- **Code Examples:** [../examples/](../examples/) directory

## 📝 License & Attribution

Time Warp Classic - Multi-language Retro IDE  
Copyright © 2025–2026 Honey Badger Universe

See [../License.md](../License.md) for full license information.

---

**Last Updated:** February 2026  
**Version:** 1.3.0 Stable

**Start with [QUICK_START.md](QUICK_START.md) →**
