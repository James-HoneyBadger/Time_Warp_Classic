    ...
    if "def update_language_indicator(self):" in content:
        ...
        try:
            ...
            if old_indicator in content:
                ...
                print(
                    "⚠️ Language indicator method structure different - may need manual fix"
                )
                return False
        else:
...
def main():
    ...
    print("=" * 20)
    print(
        f"{'✅' if theme_fixed else '⚠️'} Theme application: {'FIXED' if theme_fixed else 'NEEDS ATTENTION'}"
    )
    print(
        f"{'✅' if graphics_checked else '⚠️'} Graphics integration: {'VERIFIED' if graphics_checked else 'NEEDS WORK'}"
    )
    print(
        f"{'✅' if indicator_fixed else '⚠️'} Language indicator: {'FIXED' if indicator_fixed else 'NEEDS ATTENTION'}"
    )
